...
function applyWidgetSize(w, h, save = true) {
    ...
    if (window.AndroidBridge) {
        maxW = Math.max(window.screen.width || 0, window.screen.availWidth || 0, 1920);
        maxH = Math.max(window.screen.height || 0, window.screen.availHeight || 0, 1080);
    } else {
        maxW = Math.min(window.innerWidth - 10, window.innerWidth);
        maxH = Math.min(window.innerHeight - 10, window.innerHeight);
    }
...
function showWidget(e) {
    ...
    if (window.AndroidBridge && window.AndroidBridge.updateWindowBounds) {
        const savedW = parseInt(safeStorage.getItem('openclaw_countdown_width'), 10) || 735;
        const savedH = parseInt(safeStorage.getItem('openclaw_countdown_height'), 10) || 510;
        const curX = parseFloat(floatingWidget.dataset.x || 15);
        const curY = parseFloat(floatingWidget.dataset.y || 15);
        window.AndroidBridge.updateWindowBounds(curX, curY, savedW, savedH);
    }
}
...
function minimizeToBubble(e) {
    ...
    if (window.AndroidBridge && window.AndroidBridge.updateWindowBounds) {
        const curX = parseFloat(floatingWidget.dataset.x || 15);
        const curY = parseFloat(floatingWidget.dataset.y || 15);
        window.AndroidBridge.updateWindowBounds(curX, curY, 70, 70);
    }
}
...
function makeDraggable(element, handleElement) {
    ...
    const dragStart = (e) => {
        ...
        const pos = getPos(e);
        lastX = pos.screenX || pos.x;
        lastY = pos.screenY || pos.y;
        totalMoved = 0;
        isDragging = true;
        ...
        const dragMove = (e) => {
            ...
            const p = getPos(e);
            const curX = p.screenX || p.x;
            const curY = p.screenY || p.y;
            const dx = curX - lastX;
            const dy = curY - lastY;

            totalMoved += Math.hypot(dx, dy);
            if (totalMoved > 3 && e.cancelable) e.preventDefault();

            if (window.AndroidBridge && window.AndroidBridge.moveWindow) {
                if (Math.abs(dx) > 0 || Math.abs(dy) > 0) {
                    window.AndroidBridge.moveWindow(Math.round(dx), Math.round(dy));
                    lastX = curX;
                    lastY = curY;
                }
            } else {
                let currentX = (parseFloat(element.dataset.x) || 0) + dx;
                let currentY = (parseFloat(element.dataset.y) || 0) + dy;
...
function makeResizable(element, handleElement) {
    ...
    const resizeStart = (e) => {
        ...
        const pos = getPos(e);
        startX = pos.screenX || pos.x;
        startY = pos.screenY || pos.y;
        ...
        const resizeMove = (e) => {
            ...
            if (window.AndroidBridge && window.AndroidBridge.updateWindowBounds) {
                const curXPos = parseFloat(element.dataset.x || 15);
                const curYPos = parseFloat(element.dataset.y || 15);
                window.AndroidBridge.updateWindowBounds(curXPos, curYPos, Math.round(newW), Math.round(newH));
            }
        };
